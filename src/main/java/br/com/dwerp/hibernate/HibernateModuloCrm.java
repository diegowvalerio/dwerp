package br.com.dwerp.hibernate;

import java.io.Serializable;

import javax.enterprise.context.Dependent;

import br.com.dwerp.dao.DAOModuloCrm;
import br.com.dwerp.entidade.ModuloCrm;
import br.com.dwerp.hibernate.generico.DAOGenericoHibernate;

@Dependent
public class HibernateModuloCrm extends DAOGenericoHibernate<ModuloCrm> implements DAOModuloCrm,Serializable {
	private static final long serialVersionUID = 1L;
	
	public HibernateModuloCrm(){
		super(ModuloCrm.class);
	}


}
